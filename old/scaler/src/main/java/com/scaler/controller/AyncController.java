...
import com.scaler.service.PlanService;
...
public class AyncController {
	...
	public Object downdata(Authentication authentication) {
		if (authentication == null) {
			...
			json.put("error", "Not log in! Please log in.");
			return json;
		} else {
			JSONObject json = new JSONObject();
//			Map map = new Map();
			User theUser = userService.findByName(authentication.getName());
			List<Plan> plans = planService.findByUserId(theUser.getId());
//			System.out.print(plans.toString());
			json.put("days", theUser.getDays());
	...
	@ResponseBody
	public Object updata(Authentication authentication, @ModelAttribute User user) {
		if (authentication == null) {
			JSONObject json = new JSONObject();
			json.put("error", "Not log in! Please log in.");
			return json;
			...
			if (theUser.getName().equals(user.getName())) {
				...
				json.put("error",
						"User data does not match! Please try to log in again or contact your administrator.");
			}
			return json;
		}
	}
